...
avifBool avifPrepareReformatState(avifImage * image, avifRGBImage * rgb, avifReformatState * state)
{
    ...
    if (image->profileFormat == AVIF_PROFILE_FORMAT_NCLX) {
        ...
        } else if (image->nclx.matrixCoefficients == AVIF_NCLX_MATRIX_COEFFICIENTS_YCGCO) {
    ...
    for (uint32_t j = 0; j < image->height; ++j) {
        ...
        for (uint32_t i = 0; i < image->width; ++i) {
            ...
            if (state->mode == AVIF_REFORMAT_MODE_IDENTITY) {
                ...
                // Formulas 47-50 from https://www.itu.int/rec/T-REC-H.273-201612-I/en
                float t = Y - Cb; // (47)
                G = Y + Cb;       // (48)
                B = t - Cr;       // (49)
                R = t + Cr;       // (50)
...
avifResult avifImageYUVToRGB(avifImage * image, avifRGBImage * rgb)
{
    ...
    if (state.mode == AVIF_REFORMAT_MODE_IDENTITY) {
        ...
        // TODO: Add more fast paths for YCgCo
    } else {
        if (image->depth > 8) {
            // yuv:u16
