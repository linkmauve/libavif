...
static avifBool isIdentityMatrixCoeffs(avifReformatState * state)
{
    const float EPSILON = 0.00001f;
    if ((fabs(state->kr) < EPSILON) && (fabs(state->kg) < EPSILON) && (fabs(state->kb) < EPSILON)) {
        return AVIF_TRUE;
    }
    return AVIF_FALSE;
}
...
avifBool avifPrepareReformatState(avifImage * image, avifRGBImage * rgb, avifReformatState * state)
{
    ...
    state->identity = isIdentityMatrixCoeffs(state);
...
static int yuvToUNorm(int chan, avifRange range, int depth, float maxChannel, float v, avifBool identity)
{
    if (identity) {
        ...
        for (uint32_t outerI = 0; outerI < image->width; outerI += 2) {
                ...
                for (int bI = 0; bI < blockW; ++bI) {
                    ...
                    if (state.yuvChannelBytes > 1) {
                        ...
                        *pY = (uint16_t)yuvToUNorm(
                            AVIF_CHAN_Y, image->yuvRange, image->depth, yuvMaxChannel, yuvBlock[bI][bJ].y, state.identity);
                        if (!state.formatInfo.chromaShiftX && !state.formatInfo.chromaShiftY) {
                            ...
                            *pU = (uint16_t)yuvToUNorm(
                                AVIF_CHAN_U, image->yuvRange, image->depth, yuvMaxChannel, yuvBlock[bI][bJ].u, state.identity);
                            uint16_t * pV = (uint16_t *)&yuvPlanes[AVIF_CHAN_V][(i * 2) + (j * yuvRowBytes[AVIF_CHAN_V])];
                            *pV = (uint16_t)yuvToUNorm(
                                AVIF_CHAN_V, image->yuvRange, image->depth, yuvMaxChannel, yuvBlock[bI][bJ].v, state.identity);
                        }
                    } else {
            ...
            if (state.formatInfo.chromaShiftX && state.formatInfo.chromaShiftY) {
                ...
                if (state.yuvChannelBytes > 1) {
                    ...
                    *pU = (uint16_t)yuvToUNorm(AVIF_CHAN_U, image->yuvRange, image->depth, yuvMaxChannel, avgU, state.identity);
                    uint16_t * pV = (uint16_t *)&yuvPlanes[AVIF_CHAN_V][(uvI * 2) + (uvJ * yuvRowBytes[AVIF_CHAN_V])];
                    *pV = (uint16_t)yuvToUNorm(AVIF_CHAN_V, image->yuvRange, image->depth, yuvMaxChannel, avgV, state.identity);
                } else {
                    yuvPlanes[AVIF_CHAN_U][uvI + (uvJ * yuvRowBytes[AVIF_CHAN_U])] =
                        (uint8_t)yuvToUNorm(AVIF_CHAN_U, image->yuvRange, image->depth, yuvMaxChannel, avgU, state.identity);
                    yuvPlanes[AVIF_CHAN_V][uvI + (uvJ * yuvRowBytes[AVIF_CHAN_V])] =
                        (uint8_t)yuvToUNorm(AVIF_CHAN_V, image->yuvRange, image->depth, yuvMaxChannel, avgV, state.identity);
                }
